...
def init_paramaters():
    ...
    # 第一层的参数
    w1 = tf.Variable(tf.random.truncated_normal([784, 256], stddev=0.1))  # [784,256]的原因是图片数据是[b, 784]，要符合矩阵的乘法规则[b, 784]@[784, 256]
...
def train_epoch(epoch, train_dataset, w1, b1, w2, b2, w3, b3, lr=0.001):#只训练一遍，共60000组数据
    ...
    for step, (x, y) in enumerate(train_dataset): #enumerate将train_dataset的数据读出来，自动生成( step (200条x) (200条y) )的list的索引序列，每一个step有200条，也即一次循环，直到将所有数据读满（60000条）
        with tf.GradientTape() as tape:#用于保存计算模型的信息，便于反复求导计算
            h1 = x @ w1 + tf.broadcast_to(b1, (x.shape[0], 256))# 第一层计算， [b, 784]@[784, 256] + [256] => [b, 256] + [256] => [b,256] + [b, 256]
            h1 = tf.nn.selu(h1)  # 通过激活函数relu，构成非线性关系。tf.nn是一个tf库，提供神经网络相关操作的支持，包括卷积操作（conv）、池化操作（pooling）、归一化、loss、分类操作、embedding、RNN、Evaluation。
            h2 = h1 @ w2 + b2# 第二层计算， [b, 256] => [b, 128]
            h2 = tf.nn.selu(h2)  # 通过激活函数relu，构成非线性关系
            out = h2 @ w3 + b3# 输出层计算， [b, 128] => [b, 10]
            loss_square = tf.square(y - out)   #计算网络输出与标签之差的平方
            loss = tf.reduce_mean(loss_square)# 求阵列loss的总均值，由矩阵得到一个数值，为均方差（标准差）
            grads = tape.gradient(loss, [w1, b1, w2, b2, w3, b3])  #【反向传播】自动更新梯度，需要求梯度的张量有[w1, b1, w2, b2, w3, b3]。gradient作用：返回列表表示各个变量的梯度值，和source中的变量列表一一对应，表明这个变量的梯度。
