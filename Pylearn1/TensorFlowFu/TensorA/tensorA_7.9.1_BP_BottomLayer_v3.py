...
# tensorboard --logdir=D:\boyLen\py\Pylearn1\logs\gradient_tape2\ --host=192.168.3.38
# 浏览器：http://192.168.3.38:6006/
...
class NeuralNetwork:
    ...
    def train_neural(self, x_train, y_train, train_epochs, train_batch=1000, rate_learn=0.0075664):
        list_mse = []
        summary_writer = tf.summary.create_file_writer('D:/boyLen/py/Pylearn1/dataLen/Keras/summary1')  # Summary可视化组件初始化
        for i_batch in range(train_epochs):
            ...
            with train_summary_writer.as_default():  # 调用TensorBoard
                tf.summary.scalar('Loss', mse_single, step=i_batch)
                tf.summary.histogram('Y', layer_last['out_all_batch'].numpy(), step=i_batch)
        return list_mse
