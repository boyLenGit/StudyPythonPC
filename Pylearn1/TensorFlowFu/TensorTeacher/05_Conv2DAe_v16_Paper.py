...
path_segy_file_be_forecast = 'J:/new4.sgy'
...
def forecast_LiZhe():
    ...
    """
    # ****** 初始化变量 ******
    dimension2d_input, kernel_size, kernel_step, to_be_one = 64, 64, 8, 10
    data_origin = LenSegy.len_read_segy_file_only_trace(path_segy_file_be_forecast)['trace_raw'][:]
    ...
    data_fusion = tf.reshape(data_fusion, [-1, dimension2d_input, dimension2d_input])
    data_fusion = (data_fusion.numpy()) * to_be_one
    data_fusion = LenDataProcess.len_kernel_2d_merge(data_fusion, data_origin, dimension_kernels, size=kernel_size, step=kernel_step)
