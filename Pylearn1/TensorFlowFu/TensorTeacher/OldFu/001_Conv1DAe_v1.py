...
def get_data():
    ...
    x_train = tf.stack([x_train, x_train], axis=2)
...
class One_Conv(keras.Model):
    def __init__(self):
        ...
        self.sq1 = Sequential([
            layers.Conv1D(filters=40, kernel_size=130, activation='relu'),
            layers.MaxPool1D(pool_size=2),
            layers.Conv1D(filters=40, kernel_size=65, activation='relu'),
            layers.MaxPool1D(pool_size=2),
            layers.Conv1D(filters=40, kernel_size=65, activation='relu'),
            layers.UpSampling1D(size=2),
            layers.Conv1D(filters=40, kernel_size=65, activation='relu'),
            layers.UpSampling1D(size=2),
            layers.Conv1D(filters=1, kernel_size=130, activation='tanh'),
        ])
...
def train(db_train, train_time=100):
    model1 = One_Conv()
    model1.build(input_shape=(4, 2, 784))
    print('-----shape5:')
    model1.summary()
