# 实现论文内容
# → 梯度爆炸BUG，预测值小于0，导致值全部唯一（tanh为-1，relu为0，都代表预测值小于0；）。
# 下次测试图片是不是这样，是不是值的范围的原因
...
def get_data_v4_segy():
    ...
    x_train, x_test = segy_data_reduce_v2[0:60000].astype(numpy.float32) / to_be_one \
        , segy_data_reduce_v2[60000:].astype(numpy.float32) / to_be_one
...
def train(db_train, db_test, train_time=100):
    ...
    for epoch1 in range(train_time):
        ...
        x_test, x_test_result = x_test.numpy() * to_be_one, x_test_result.numpy() * to_be_one
        # ↓ 绘图
