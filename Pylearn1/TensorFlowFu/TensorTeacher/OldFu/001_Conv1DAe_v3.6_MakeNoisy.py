...
def get_data_v4_segy():
    ...
    # 生成带噪声数据
    x_train_noisy, x_test_noisy = len_make_noisy(x_train.copy()), len_make_noisy(x_test.copy())
    # 数据类型转化与缩放
    ...
    x_train_noisy, x_test_noisy = tf.reshape(x_train_noisy, (-1, 456)), tf.reshape(x_test_noisy, (-1, 456))
...
def train(db_train, db_test, train_time=100):
    model1 = One_Conv()
    model1.build(input_shape=(None, 456, input_data_dim))
