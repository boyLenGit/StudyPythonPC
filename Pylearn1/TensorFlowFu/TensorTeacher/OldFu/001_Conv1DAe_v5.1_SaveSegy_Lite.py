...
def get_data_v4_segy():
    ...
    x_test_all_data = segy_data[0:20000]  # 复制一遍原始数据
    # Cut数据
    x_train, x_test = segy_data_reduce[0:7000], segy_data_reduce[-1000:]
    # 【Lite】原始数据
...
def len_save_segy(input_list, read_path, save_path):  # 输入list和path即可。目前只能用与原文件同的规模的数据来生成segy，尚不可以用任意规模的数据生成segy
    ...
    with segyio.open(read_path) as src:
        print('@开始保存新Segy文件，原文件共{0}条数据，单条长{1}；新内容共{2}条数据，单条长{3}。'.format(len(src.trace), len(src.samples), len(input_list), len(input_list[0])), end='')
        spec = segyio.spec()
...
def boylen_train(db_train, db_test, db_all, train_time=1):
    # 初始化数据
    global len_origin_single_data
    # ↓ 初始化并训练模型
    model1 = One_Conv()
    ...
    for epoch1 in range(train_time):
        ...
        print('@ shape1:', len(x_test_list[0]), len(x_test_result_list[0]), len(x_test_noisy_list[0]))
        print('@ shape2', tf.shape(x_test_list), tf.shape(x_test_result_list), tf.shape(x_test_noisy_list))
    ...
    for step, (x_all, x_all_noisy) in enumerate(db_all):
        ...
        print('shape4:', tf.shape(x_all_result_single))
    ...
    print('\n@ shape3', tf.shape(list_result_all_fusion), tf.shape(list_noisy_all_fusion), tf.shape(list_input_all_fusion))
    list_result_all_fusion = tf.reshape(list_result_all_fusion, [-1, len_origin_single_data])
    list_noisy_all_fusion = tf.reshape(list_noisy_all_fusion, [-1, len_origin_single_data])
    list_input_all_fusion = tf.reshape(list_input_all_fusion, [-1, len_origin_single_data])
    list_result_all_fusion, list_noisy_all_fusion, list_input_all_fusion = (list_result_all_fusion.numpy()) * to_be_one, (list_noisy_all_fusion.numpy()) * to_be_one, (list_input_all_fusion.numpy()) * to_be_one
    print('@原始数据规模：', list_result_all_fusion.shape, list_noisy_all_fusion.shape, list_input_all_fusion.shape)
    ...
    progress1 = multiprocessing.Process(target=len_save_segy, name='001', args=(list_noisy_all_fusion, 'E:/Research/data/F3_entire.segy', 'J:/test_noisy.segy'))
    progress2 = multiprocessing.Process(target=len_save_segy, name='002', args=(list_result_all_fusion, 'E:/Research/data/F3_entire.segy', 'F:/test_result.segy'))
    progress1.start()
    progress2.start()
    print('@多进程运行成功')
