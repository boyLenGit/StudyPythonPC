...
def get_data_v4_segy():
    ...
    x_test_all_data = segy_data[0:10000]  # 复制一遍原始数据
    # 切割训练与测试数据
    x_train, x_test = segy_data_reduce[0:70000], segy_data_reduce[-1000:]
    # 保存带噪声文件
...
def boylen_train(db_train, db_test, db_all, train_time=100):
    # 初始化数据
    global len_origin_single_data
    # ↓ 初始化并训练模型
    model1 = One_Conv()
    ...
    for step, (x_all, x_all_noisy) in enumerate(db_all):
        ...
        list_result_all_fusion.append(x_all_result_single)
        list_noisy_all_fusion.append(x_all_noisy)
        list_input_all_fusion.append(x_all)
        if step % 100 == 0:
            print('\b\b\b\b\b', '%02d' % (100*step//len_data_all), '%',  end='')
    # 预测数据单条全部融合完毕，画个图看看效果
    list_result_all_fusion, list_noisy_all_fusion, list_input_all_fusion = tf.reshape(list_result_all_fusion, [-1, len_origin_single_data])\
    ...
    '''len_save_segy(list_noisy_all_fusion, read_path='E:/Research/data/F3_entire.segy', save_path='F:/test_noisy.segy')
    len_save_segy(list_result_all_fusion, read_path='E:/Research/data/F3_entire.segy', save_path='F:/test_result.segy')'''

    multiprocessing.Process(target=len_save_segy, name='001', args=(list_noisy_all_fusion, 'E:/Research/data/F3_entire.segy', 'F:/test_noisy.segy')).run()
    multiprocessing.Process(target=len_save_segy, name='002', args=(list_result_all_fusion, 'E:/Research/data/F3_entire.segy', 'F:/test_result.segy')).run()
