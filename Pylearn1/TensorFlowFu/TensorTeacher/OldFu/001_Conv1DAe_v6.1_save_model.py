...
path_model_weights = 'D:/boyLen/Python/Pylearn1/TensorFlowFu/TensorTeacher/data/001_Conv1DAE_v6.1.Len90'
...
def get_data_v4_segy():
    ...
    # LenSegy.len_get_segy_average_max_value(segy_data)
    # ------ test ------
    print(len(segy_data))
    # ------
    segy_data_reduce, x_test_all_data = segy_data[:], segy_data[:1000]  # 复制原数据
    # 切割训练与测试数据
    x_train, x_test = segy_data_reduce[:3000], segy_data_reduce[-1000:]
    # 【Lite】原始数据
...
def boylen_kernel_generator_for_train(input_list):  # 【核采样】指定宽度采样法（仅对x_train进行处理即可）
    ...
    print('')
    print('@ 1==', tf.shape(enlarge_data_list))
...
def boylen_train(db_train, db_test, db_all, train_time=100):
    ...
    for epoch1 in range(train_time+90):
        ...
        if epoch1 % 5 == 0:
            ...
            model1.save_weights(path_model_weights+'{0}'.format(epoch1))
    # ------------------------------------- 生成训练完预测出来的segy数据 ---------------------------------------
    ...
    # 保存训练的模型
    # tf.saved_model.save(model1, path_model)
    # tf.compat.v1.keras.experimental.export_saved_model(model1, path_model)
    # model1.save(path_model)
    model1.save_weights(path_model_weights)
