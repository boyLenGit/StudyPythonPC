...
def get_data_v4_segy():
    with segyio.open(path_segy_file, mode='r+') as F3_entire:
    ...
    print(len(segy_data))
    # ------
    segy_data_reduce, x_test_all_data = segy_data[:], segy_data[:3000]  # 复制原数据
    # 切割训练与测试数据
    x_train, x_test = segy_data_reduce[:100000], segy_data_reduce[-1000:]
    # 【Lite】原始数据
