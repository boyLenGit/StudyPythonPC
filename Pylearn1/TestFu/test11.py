...
class Layer:
    ...
    def activate(self, x): #x：[1.75278051 0.49179304]
        # 前向传播函数
        r = np.dot(x, self.weights) + self.bias  # X@W+b
        # 通过激活函数，得到全连接层的输出o
        self.last_activation = self._apply_activation(r)
        return self.last_activation     #self.last_activation的规模跟每层的网络数量有关
...
class NeuralNetwork:
    ...
    # 前向传播
    def feed_forward(self, X):  #X:[1.75278051 0.49179304]
        for layer in self._layers:
            # 依次通过各个网络层
            X = layer.activate(X)   #X:[0.57680817 0.47758824]
        return X
    ...
    def train(self, X_train, X_test, y_train, y_test, learning_rate, max_epochs):
        ...
        mses = [];        accuracys = []
        for i in range(max_epochs):  # 训练1000次epoch
            for j in range(len(X_train)):  # 一次训练一个样本
                self.backpropagation(X_train[j], y_onehot[j], learning_rate)
            y_out =self.feed_forward(X_train)
            mse = np.mean(np.square(y_onehot - y_out))
            print('Train:', y_out)
            mses.append(mse)
...
def main():
    ...
    print(':::::', X_train.shape, y_train.shape, y_train.shape[0])
    print('-----', X_train, y_train)
    mses, accuracys = nn.train(X_train, X_test, y_train, y_test, 0.01, 100)

    x = [i for i in range(0, len(mses))]
