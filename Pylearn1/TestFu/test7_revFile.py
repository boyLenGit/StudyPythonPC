...
def ModFun_CreateAndInsertData_64W(tablename64W,string_file_Full):
    ...
    try:
        cursor1.execute(string_Create_Full.format(tablename2=tablename64W))
    except sqlite3.OperationalError:
        print('数据表 {tablename2} 已经存在！'.format(tablename2=tablename64W))
    try:
        query = string_Insert_Full.format(tablename2=tablename64W)  # 导入数据库用
...
def ModFun_SmartBuildSQLiteExecute(string_file):
    ...
    string_Create_First = 'create table {tablename2}';    string_Create_Second =  ' (id TEXT primary key,'
    string_Insert_First = 'insert into {tablename2} values';    string_Insert_Second = '(?,'
    ...
    for num1 in ListOfFor :
        ...
        if(num1 != ListOfFor[LenOfSQL-1]):
            string_Create_Second += ',';            string_Insert_Second += ','
    string_Create_Second += ')';    string_Insert_Second += ')'
...
def ModFun_RecvFile_TCP_Gen2(string_FilePath_First,string_host,string_port,TranSpeed_Single):
    socket1 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    socket1.bind((string_host, string_port))  # 绑定地址到套接字
    socket1.listen(5);print('等待来自客户端的连接')
    while True:
        OutSock1, addr1 = socket1.accept();print('收到来自', addr1,'的连接请求，即将开始传输文件')
        Recvdata1 = OutSock1.recv(1024).decode()
        RecvdataList1 = Recvdata1.split('^')
        data1=RecvdataList1[0];        TranSpeed_Single = RecvdataList1[1];        string_FilePath_Second = RecvdataList1[2] #将传来的文件信息分离成各个变量（路径、文件名等）
        file_total_size = int(data1)
        PersentOfFileSize = file_total_size/60;PersentOfFileSizeIF=PersentOfFileSize
        receivefileSize=0;        CntOfProgressBar = 0
        OutSock1.send('服务端已接收'.encode())
        print('全部传输进度：||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n当前传输进度：',end='')
        ...
        if b'102455231' in data_end:
            file3.close()
            OutSock1.close();socket1.close()
            print('\n接收完毕，文件大小：',receivefileSize/1000,'KB','；存储路径：',string_file_Full)
            return string_file_Full

def RecvFile_TCP_Gen2():
    socket1 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    socket1.bind((host, port))  # 绑定地址到套接字
    socket1.listen(5);print('等待来自客户端的连接')
    while True:
        OutSock1, addr1 = socket1.accept();print('连接来自：', addr1)
        Recvdata1 = OutSock1.recv(1024).decode()
        RecvdataList1 = Recvdata1.split('^')
        data1=RecvdataList1[0];        TranSpeed_Single = RecvdataList1[1];        string_FilePath_Second = RecvdataList1[2] #将传来的文件信息分离成各个变量（路径、文件名等）
        file_total_size = int(data1);print('data1:',data1,'TranSpeed_Single:',TranSpeed_Single)
        PersentOfFileSize = file_total_size/60;PersentOfFileSizeIF=PersentOfFileSize
        receivefileSize=0
        OutSock1.send('服务端已接收'.encode())
        print('全部传输进度：||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||\n当前传输进度：',end='')
        string_file_Full = string_StoreFilePath_First+string_FilePath_Second
        with open(string_file_Full,'wb') as file3:      #循环接收文件主体部分
            while receivefileSize<file_total_size:
                data2 = OutSock1.recv(int(TranSpeed_Single))
                file3.write(data2)
                ...
                if receivefileSize > PersentOfFileSizeIF:
                    ...
                    PersentOfFileSizeIF+=PersentOfFileSize
        data_end = OutSock1.recv(1024)
        if b'132' in data_end:
            file3.close()
            OutSock1.close();socket1.close()
            print('|\n接收完毕，文件大小：',receivefileSize/1000,'KB','；接收路径：',string_file_Full)

def ControlCenter():
    string_file_Full1 = ModFun_RecvFile_TCP_Gen2(string_StoreFilePath_First,host,port,TranSpeed_Single)
    ModFun_CreateAndInsertData_64W('test3',string_file_Full1)
    pass

if __name__ == '__main__':
    Progress1 = multiprocessing.Process(target=ControlCenter,name='boyLenRecv');    Progress1.start()
    Progress1.join();    print('文件传输进程关闭')